...
class VoiceMaster(commands.Cog):
    ...
    async def summon(self, ctx):
        ...
        try:
            ...
            await ctx.send(
                "❌ Could not establish a stable voice connection after 3 attempts. "
                "This is likely a Discord voice routing issue on the host — try again in a minute."
            )
            logger.error(f"Summon failed after retries in guild {ctx.guild.id}: {last_error}")

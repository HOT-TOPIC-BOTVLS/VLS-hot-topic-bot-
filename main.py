...
async def load_cogs():
    ...
    for cog_file in cogs_dir.glob("*.py"):
        ...
        if cog_name in ["main", "config", "database", "logger", "dashboard"]:
            continue
...
async def start_health_server():
    """Health check server on port 10000 (keeps Render happy)."""
    app = web.Application()
...
def run_dashboard():
    """Run FastAPI dashboard on port 8080 in a background thread."""
